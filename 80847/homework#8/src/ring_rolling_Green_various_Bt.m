...
% Ns/m = 10^3 g/s; 20000 g/s = 20 Ns/m
%sBt =  20000; dirname = 'ring_rolling_Green_damping_20000';
%%Bt =  2000; dirname = 'ring_rolling_Green_damping_2000';
%Bt =  200; dirname = 'ring_rolling_Green_damping_200';
%%Bt =  20; dirname = 'ring_rolling_Green_damping_20';
Bt =  2; dirname = 'ring_rolling_Green_damping_2';
...
tf = 5.0;
interval = [0, tf];
ring_free = @(t,q) ring_free_param(t,q, ring, Bt);
...
for t = 0:0.1:tf
    ...
    pbaspect([1.5 1 1]);
    grid on;
    filename = strcat(dirname, '/deform_', num2str(floor(1000*t),'%04d'), '.png');
...
v = VideoWriter(dirname, 'MPEG-4');
open(v);
writeVideo(v, M);
close(v);

function dotq = ring_free_param(t,q, body, Bt)
    %disp(t);
    disp(num2str(t,"%8.6f"));
