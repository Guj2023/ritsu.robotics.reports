...
for t = 0:0.1:tf
    ...
    pbaspect([1.5 1 1]);
    grid on;
    filename = strcat('ring_rolling_Green/deform_', num2str(floor(1000*t),'%04d'), '.png');
...
v = VideoWriter('ring_rolling_Green', 'MPEG-4');
open(v);
writeVideo(v, M);
close(v);
...
function dotq = ring_free_param(t,q, body)
    ...
    if isempty(npoints)
        ...
        Bcontact = 0;
        friction_damping = 20000; % Ns/m = 10^3 g/s; 20000 g/s = 20 Ns/m
    end
