classdef Body
    ...
    methods
        ...
        function [ outer_contour, inner_contours ] = outer_and_inner_contours(obj)
            outer_contour  = [];
            inner_contours = [];
            for ct=1:obj.numContours
                if obj.Contours(ct).Area > 0
                    outer_contour = [ outer_contour; ct ];
                else
                    inner_contours = [ inner_contours; ct ];
                end
            end
        end
        
        function forces = nodal_forces_equivalent_to_pressure(obj, contours, positions, pressure)
            forces = zeros(2, obj.numNodalPoints);
            for ct = contours
                forces = forces + obj.Contours(ct).nodal_forces_equivalent_to_pressure(obj.numNodalPoints, positions, pressure);
            end
        end
